...
### Icons Download Directory

The `_data/iconsDownload/` directory contains the actual icon image files downloaded from IPFS CIDs referenced in the `_data/icons/` JSON files. These files are used for local processing and validation. Each file is named by its IPFS CID (e.g., `QmAbc123...`).

**Note:** Icon files MUST be less than 250kb in size as specified in the icon JSON files.
